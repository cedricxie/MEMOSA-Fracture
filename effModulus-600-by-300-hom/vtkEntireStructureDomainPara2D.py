...
def dumpvtkEntireStructureDomain(geomFields, nmesh, meshesLocal, meshesGlobal, mtype, sFields, file_name, title_name,nstep):
  ...
  distortionXFields = []
  for n in range(0,nmesh):
     distortionXFields.append( sFields.distortionX[cellSitesLocal[n]].asNumPyArray() )
     
  distortionYFields = []
  for n in range(0,nmesh):
     distortionYFields.append( sFields.distortionY[cellSitesLocal[n]].asNumPyArray() )
     
  distortionZFields = []
  for n in range(0,nmesh):
     distortionZFields.append( sFields.distortionZ[cellSitesLocal[n]].asNumPyArray() )
  ...
  strainYFieldGlobal = []
  strainZFieldGlobal = []
  distortionXFieldGlobal = []
  distortionYFieldGlobal = []
  distortionZFieldGlobal = []
  tractXFieldGlobal = []
  tractYFieldGlobal = []
  tractZFieldGlobal = []
  JoneFieldsGlobal = []
  ...
  for n in range(0,nmesh):
    ...
    distortionXFieldGlobal.append( zeros((selfCount,3), float) ) #if it is velocity (selfCount,3)
    distortionYFieldGlobal.append( zeros((selfCount,3), float) ) #if it is velocity (selfCount,3)
    distortionZFieldGlobal.append( zeros((selfCount,3), float) ) #if it is velocity (selfCount,3)
    ...
    dXFieldGlobal  = distortionXFieldGlobal[n]  
    dYFieldGlobal  = distortionYFieldGlobal[n]
    dZFieldGlobal  = distortionZFieldGlobal[n]
    ...
    dXFieldLocal   = distortionXFields[n]
    dYFieldLocal   = distortionYFields[n]
    dZFieldLocal   = distortionZFields[n]
    ...
    for i in range(0,selfCount):
       ...
       dXFieldGlobal[globalID] = dXFieldLocal[i]
       dYFieldGlobal[globalID] = dYFieldLocal[i]
       dZFieldGlobal[globalID] = dZFieldLocal[i]
    ...
    MPI.COMM_WORLD.Allreduce(MPI.IN_PLACE, [dXFieldGlobal,MPI.DOUBLE], op=MPI.SUM)    
    MPI.COMM_WORLD.Allreduce(MPI.IN_PLACE, [dYFieldGlobal,MPI.DOUBLE], op=MPI.SUM)
    MPI.COMM_WORLD.Allreduce(MPI.IN_PLACE, [dZFieldGlobal,MPI.DOUBLE], op=MPI.SUM)
  ...
  if MPI.COMM_WORLD.Get_rank() == 0:
     ...
     f.write("VECTORS "+"DistortX"+" double\n")
     dXFieldGlobal = distortionXFieldGlobal[n]
     for i in range(0,ncell):
         f.write(str(dXFieldGlobal[i][0])+" "+str(dXFieldGlobal[i][1])+" "+str(dXFieldGlobal[i][2])+"\n")
         
     f.write("VECTORS "+"DistortY"+" double\n")
     dYFieldGlobal = distortionYFieldGlobal[n]
     for i in range(0,ncell):
         f.write(str(dYFieldGlobal[i][0])+" "+str(dYFieldGlobal[i][1])+" "+str(dYFieldGlobal[i][2])+"\n")
         
     f.write("VECTORS "+"DistortZ"+" double\n")
     dZFieldGlobal = distortionZFieldGlobal[n]
     for i in range(0,ncell):
         f.write(str(dZFieldGlobal[i][0])+" "+str(dZFieldGlobal[i][1])+" "+str(dZFieldGlobal[i][2])+"\n")
