...
def dumpvtkEntireFractureDomain(geomFields, nmesh, meshesLocal, meshesGlobal, mtype, tFields, sFields, file_name, title_name,nstep):
  ...
  sourceFields = []
  for n in range(0,nmesh):
     sourceFields.append( tFields.source[cellSitesLocal[n]].asNumPyArray() )   
  ...
  for n in range(0,nmesh):
    ...
    sourceFieldGlobal.append( zeros((selfCount,1), float) ) #if it is velocoity (selfCount,3) 
    ...
    for i in range(0,selfCount):
       ...
       srcFieldGlobal[globalID] = srcFieldLocal[i]
    ...
    MPI.COMM_WORLD.Allreduce(MPI.IN_PLACE, [srcFieldGlobal,MPI.DOUBLE], op=MPI.SUM)
  ...
  if MPI.COMM_WORLD.Get_rank() == 0:
     ...
     f.write("SCALARS "+"SourceField"+" double\n")
     f.write("LOOKUP_TABLE default\n")
     for i in range(0,ncell):
         f.write(str(srcFieldGlobal[i][0])+"\n") 
