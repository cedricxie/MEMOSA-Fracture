...
E = 5.2e9                    # Matrix Young's Modulus
...
cLoC=  5e-5                    # model parameter controlling the width of the smooth approximation of the crack, m
...
# Set Random Crack Parameters
numCrack = 300
maxCrackLength = 4.0*cLoC
minCrackLength = 2.0*cLoC
maxCrackWidth = 2.0*cLoC
minCrackWidth = 1.0*cLoC
...
PF_min = array([1.0])
PF_mini = array([0.0])
PF_min_energy = array([0.0,0.0,0.0,0.0])
PF_change_max= array([0.0])
PF_change_maxi= array([0.0])
...
crack_length = []
crack_angle = []
################Random Crack Generation################
for j in range(0,numCrack):
    #print "checking crack #",j
    crack_center_x.append(0)
    crack_center_y.append(0)
    crack_width.append(0)
    crack_length.append(0)
    crack_angle.append(0)

    crack_center_verified_flag=0
    while (crack_center_verified_flag==0):
        print "checking crack #",j
        crack_center_verified_flag=1
        ...
        #print "crack waiting to be verified ",crack_center_x[j],crack_center_y[j],crack_width[j],crack_length[j],crack_angle[j]
        for i in range(0,Count):
            distance1 = fabs(tan(crack_angle[j])*coordA[i,0]+(-1.0)*coordA[i,1]+(-tan(crack_angle[j])*crack_center_x[j]+crack_center_y[j]))\
            /sqrt(tan(crack_angle[j])**2.0+1)
            distance2 = fabs(tan(crack_angle[j]+pi/2.0)*coordA[i,0]+(-1.0)*coordA[i,1]+(-tan(crack_angle[j]+pi/2.0)*crack_center_x[j]+crack_center_y[j]))\
            /sqrt(tan(crack_angle[j]+pi/2.0)**2.0+1)
            if (distance1<crack_width[j]/2.0 and distance2<crack_length[j]/2.0):
                if (randomCrackField[i]==0):
                    randomCrackField[i]=1
                    PFHistoryField[i]=0.0
                    pfperfectFieldsA[i]=-1
                    pfvFieldsA[i]=0.0
                    print "crack founded at ",i,coordA[i,0],coordA[i,1],distance1,distance2
                else:
                    crack_center_verified_flag=0
                    print "conflict found at",i
...
##########################################################################################                
       ...
       for i in range(0,Count):
           ...
           if coordA[i,1]> BoundaryPositionTop and coordA[i,0]<BoundaryPositionRight and coordA[i,0] > BoundaryPositionLeft and i < selfCount :
           ...
           elif coordA[i,1]< BoundaryPositionBottom and coordA[i,0]<BoundaryPositionRight and coordA[i,0] > BoundaryPositionLeft and i < selfCount :
...
# Start of the fracture model
 ...
 ##########################################################################################  
       PF_min[0] = 1.0
       PF_min_local = PF_min[0]
       PF_mini[0]=0
       PF_change_max[0]=0
       PF_change_max_local = PF_change_max[0]
       PF_change_maxi[0]=0
       PF_min_energy = array([-1e20,-1e20,-1e20,-1e20])
       for i in range(0,Count):
           ...
           if abs(PhaseFieldA[i]-PF_stored[i]) > PF_change_max[0] :
               ...
               PF_change_max_local = PF_change_max[0]
               PF_change_maxi[0]=i
           if coordA[i,0]<cLoC and coordA[i,0]>-cLoC and coordA[i,1]<cLoC+0.005 and coordA[i,1]>-cLoC+0.005:
               
              print "Energy Field in the center: ",coordA[i,0],coordA[i,1],ElasticEnergyField[i]
              print "Pincipal 1;",(2.0*G_local[i]+Lamda_local[i])*eigenvalueFieldsA[i][0] + Lamda_local[i]*eigenvalueFieldsA[i][1] + Lamda_local[i]*eigenvalueFieldsA[i][2]
              print 0.5*Lamda_local[i]*(((2.0*G_local[i]+Lamda_local[i])*eigenvalueFieldsA[i][0] + Lamda_local[i]*eigenvalueFieldsA[i][1] + Lamda_local[i]*eigenvalueFieldsA[i][2])/(2.0*G_local[i]+3.0*Lamda_local[i]))**2.0
              print "Pincipal 2;",Lamda_local[i]*eigenvalueFieldsA[i][0] + (2.0*G_local[i]+Lamda_local[i])*eigenvalueFieldsA[i][1] + Lamda_local[i]*eigenvalueFieldsA[i][2]
              print 0.5*Lamda_local[i]*((Lamda_local[i]*eigenvalueFieldsA[i][0] + (2.0*G_local[i]+Lamda_local[i])*eigenvalueFieldsA[i][1] + Lamda_local[i]*eigenvalueFieldsA[i][2])/(2.0*G_local[i]+3.0*Lamda_local[i]))**2.0
              print "Pincipal 3;",Lamda_local[i]*eigenvalueFieldsA[i][0] + Lamda_local[i]*eigenvalueFieldsA[i][1] + (2.0*G_local[i]+Lamda_local[i])*eigenvalueFieldsA[i][2]
              print 0.5*Lamda_local[i]*((Lamda_local[i]*eigenvalueFieldsA[i][0] + Lamda_local[i]*eigenvalueFieldsA[i][1] + (2.0*G_local[i]+Lamda_local[i])*eigenvalueFieldsA[i][2])/(2.0*G_local[i]+3.0*Lamda_local[i]))**2.0
       ...
       if PF_min_local != PF_min[0]:
           PF_mini[0] = 0
       else:
           PF_min_energy[0] = ElasticEnergyField[int(PF_mini[0])]
           PF_min_energy[1] = 0.5*Lamda_local[int(PF_mini[0])]*(((2.0*G_local[int(PF_mini[0])]+Lamda_local[int(PF_mini[0])])*eigenvalueFieldsA[int(PF_mini[0])][0] + Lamda_local[int(PF_mini[0])]*eigenvalueFieldsA[int(PF_mini[0])][1] + Lamda_local[int(PF_mini[0])]*eigenvalueFieldsA[int(PF_mini[0])][2])/(2.0*G_local[int(PF_mini[0])]+3.0*Lamda_local[int(PF_mini[0])]))**2.0
           PF_min_energy[2] = 0.5*Lamda_local[int(PF_mini[0])]*((Lamda_local[int(PF_mini[0])]*eigenvalueFieldsA[int(PF_mini[0])][0] + (2.0*G_local[int(PF_mini[0])]+Lamda_local[int(PF_mini[0])])*eigenvalueFieldsA[int(PF_mini[0])][1] + Lamda_local[int(PF_mini[0])]*eigenvalueFieldsA[int(PF_mini[0])][2])/(2.0*G_local[int(PF_mini[0])]+3.0*Lamda_local[int(PF_mini[0])]))**2.0
           PF_min_energy[3] = 0.5*Lamda_local[int(PF_mini[0])]*((Lamda_local[int(PF_mini[0])]*eigenvalueFieldsA[int(PF_mini[0])][0] + Lamda_local[int(PF_mini[0])]*eigenvalueFieldsA[int(PF_mini[0])][1] + (2.0*G_local[int(PF_mini[0])]+Lamda_local[int(PF_mini[0])])*eigenvalueFieldsA[int(PF_mini[0])][2])/(2.0*G_local[int(PF_mini[0])]+3.0*Lamda_local[int(PF_mini[0])]))**2.0
           print "Minimum Found",PF_min_energy,coordA[int(PF_mini[0]),0],coordA[int(PF_mini[0]),1]
           print "Pincipal 1;",(2.0*G_local[int(PF_mini[0])]+Lamda_local[int(PF_mini[0])])*eigenvalueFieldsA[int(PF_mini[0])][0] + Lamda_local[int(PF_mini[0])]*eigenvalueFieldsA[int(PF_mini[0])][1] + Lamda_local[int(PF_mini[0])]*eigenvalueFieldsA[int(PF_mini[0])][2]
           print "Pincipal 2;",Lamda_local[int(PF_mini[0])]*eigenvalueFieldsA[int(PF_mini[0])][0] + (2.0*G_local[int(PF_mini[0])]+Lamda_local[int(PF_mini[0])])*eigenvalueFieldsA[int(PF_mini[0])][1] + Lamda_local[int(PF_mini[0])]*eigenvalueFieldsA[int(PF_mini[0])][2]
           print "Pincipal 3;",Lamda_local[int(PF_mini[0])]*eigenvalueFieldsA[int(PF_mini[0])][0] + Lamda_local[int(PF_mini[0])]*eigenvalueFieldsA[int(PF_mini[0])][1] + (2.0*G_local[int(PF_mini[0])]+Lamda_local[int(PF_mini[0])])*eigenvalueFieldsA[int(PF_mini[0])][2]
           print "TractionX: ",tractXFieldsA[int(PF_mini[0])]
           print "TractionY: ",tractYFieldsA[int(PF_mini[0])]
           print "TractionZ: ",tractZFieldsA[int(PF_mini[0])]

       if PF_change_max_local != PF_change_max[0]:
           PF_change_maxi[0] = 0
       MPI.COMM_WORLD.Allreduce(MPI.IN_PLACE,[PF_mini, MPI.DOUBLE], op=MPI.MAX)
       MPI.COMM_WORLD.Allreduce(MPI.IN_PLACE,[PF_change_maxi, MPI.DOUBLE], op=MPI.MAX)
       MPI.COMM_WORLD.Allreduce(MPI.IN_PLACE,[PF_min_energy, MPI.DOUBLE], op=MPI.MAX)
       
       if rank_id==0:
           print "Phase Field Minimum Value: ",PF_min[0],PF_min_energy
           print "Maximum Phase Field Change: ",PF_change_max[0],PhaseFieldA[PF_change_maxi[0]]
           #print "Minimum Found @ rank 0!",PF_min_energy
