...
BoundaryPositionBottom = 6e-6/300.0
CrackTipX = 0.30e-6
CrackTipY = 0.30e-6
cellLen = 0.02e-6
...
cLoC=  0.04e-6                  # model parameter controlling the width of the smooth approximation of the crack, m
cFED = 2.9e6*4.0*36e-6          # critical fracture energy density, J/m^2
...
soptions.setVar("operatingTemperature",iniTemp) 
...
for n in range(0,nmesh):
    ...
    alphaFields = structureFields.alpha[cellSitesLocal[n]]
...
################Forcing perfect region################  
        ...
        if PFHistoryField[i]==1.0:
            ...
            if planeStatus == 2:
                K_local.append(9.0*K*G/(3.0*K+4.0*G))
...
for nstep in range(0,numSteps):
   ...
   #        bc['specifiedXDeformation'] = KI/2/G
   #        bc['specifiedYDeformation'] = KI/2/G
...
##########################################################################################                
       ...
       for i in range(0,Count):
           ...
           if strain_trace[i] - 3.0*alphaFieldsA[i]*(temperatureFieldsA[i]-iniTemp) > 0:
               strain_trace_positive=strain_trace[i] - 3.0*alphaFieldsA[i]*(temperatureFieldsA[i]-iniTemp)
               strain_trace_negative=0
               #V_flag[i] = 0
           else :
               strain_trace_positive=0
               strain_trace_negative=strain_trace[i] - 3.0*alphaFieldsA[i]*(temperatureFieldsA[i]-iniTemp)  
               #V_flag[i] = 1               
           strain_trace_mean=(strain_trace[i] - 3.0*alphaFieldsA[i]*(temperatureFieldsA[i]-iniTemp))/3.0
           strain_dev2_trace=(strainXFieldsA[i][0]- alphaFieldsA[i]*(temperatureFieldsA[i]-iniTemp)-strain_trace_mean)**2+strainXFieldsA[i][1]**2+strainXFieldsA[i][2]**2+\
           strainYFieldsA[i][0]**2+(strainYFieldsA[i][1]- alphaFieldsA[i]*(temperatureFieldsA[i]-iniTemp)-strain_trace_mean)**2+strainYFieldsA[i][2]**2+\
           strainZFieldsA[i][0]**2+strainZFieldsA[i][1]**2+(strainZFieldsA[i][2]- alphaFieldsA[i]*(temperatureFieldsA[i]-iniTemp)-strain_trace_mean)**2
           strain_2_trace=(strainXFieldsA[i][0]- alphaFieldsA[i]*(temperatureFieldsA[i]-iniTemp))**2+strainXFieldsA[i][1]**2+strainXFieldsA[i][2]**2+\
           strainYFieldsA[i][0]**2+(strainYFieldsA[i][1]- alphaFieldsA[i]*(temperatureFieldsA[i]-iniTemp))**2+strainYFieldsA[i][2]**2+\
           strainZFieldsA[i][0]**2+strainZFieldsA[i][1]**2+(strainZFieldsA[i][2]- alphaFieldsA[i]*(temperatureFieldsA[i]-iniTemp))**2
           ...
           if planeStatus == 1: #planeStrain
               Sigma_ZZ[i] = PhaseFieldA[i]**2.0*Lamda_local[i]*(strainXFieldsA[i][0]- alphaFieldsA[i]*(temperatureFieldsA[i]-iniTemp)+strainYFieldsA[i][1]- alphaFieldsA[i]*(temperatureFieldsA[i]-iniTemp))
               Average_Sigma_ZZ[0] += Sigma_ZZ[i]*volumeA[i] 
           if planeStatus == 2: #planeStress
               Strain_ZZ[i] = - Lamda_local[i] * ( strainXFieldsA[i][0]- alphaFieldsA[i]*(temperatureFieldsA[i]-iniTemp)+strainYFieldsA[i][1]- alphaFieldsA[i]*(temperatureFieldsA[i]-iniTemp))/(Lamda_local[i]+2.0*G_local[i])+ alphaFieldsA[i]*(temperatureFieldsA[i]-iniTemp)
               Average_Strain_ZZ[0] += Strain_ZZ[i]*volumeA[i] 

           if planeStatus == 1 and coordA[i,0]>0.0*BoundaryPositionRight and coordA[i,0]<1.0*BoundaryPositionRight\
           ...
           if planeStatus == 2 and coordA[i,0]>0.0*BoundaryPositionRight and coordA[i,0]<1.0*BoundaryPositionRight\
           and coordA[i,1]>0.0*BoundaryPositionTop and coordA[i,1]<1.0*BoundaryPositionTop and i < selfCount:
       ...
       if rank_id == 0:
           ...
           if planeStatus == 1:
               print  "Average Sigma_ZZ",Average_Sigma_ZZ[0]
           if planeStatus == 2:
