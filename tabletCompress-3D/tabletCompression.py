...
beamFront = 3
IDs = (2, 3,5,6,7,8)
BoundaryPositionTop = 4.9996e-3 #Top
BoundaryPositionRight = 1e-5 #Right
BoundaryPositionLeft = -1e-5
BoundaryPositionBottom = -4.9996e-3
...
cFED = 0.4                    # critical fracture energy density, J/m^2
cLoC=  2e-5                    # model parameter controlling the width of the smooth approximation of the crack, m
 ...
 ##########################################################################################  
       ...
       for i in range(0,Count):
           ...
           if abs(PhaseFieldA[i]-PF_stored[i]) > PF_change_max[0] :
               ...
               PF_change_max_local = PF_change_max[0]
               PF_change_maxi[0]=i
           if coordA[i,0]<cLoC and coordA[i,0]>-cLoC and coordA[i,1]<cLoC and coordA[i,1]>-cLoC:
